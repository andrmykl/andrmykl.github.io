...
.cv .list-group-item > .row > div {
  max-width: none !important;
  padding-left: 0;
  padding-right: 0;
}

.cv .list-group-item > .row > .date-column {
  width: auto;
...
.cv .date-column + div h6 {
  margin-left: 0 !important;
}

.cv .date-column table.table-cv,
.cv .date-column table.table-cv.table-dark {
  border: 0 !important;
...
.cv .date-column table.table-cv td,
.cv .date-column table.table-cv th {
  border: 0 !important;
  background: transparent !important;
  padding: 0 !important;
}
...
.cv .date-column .badge::before,
.cv .date-column .badge::after {
  content: none !important;
  display: none !important;
}

.cv .date-column p.location {
...
.cv .date-column p.location .iconlocation {
...
let setSiteAccent = () => {
  ...
  root.style.setProperty("--global-theme-color", siteAccentColor);
  root.style.setProperty("--global-hover-color", siteAccentHoverColor);
  root.style.setProperty("--global-hover-text-color", siteAccentTextColor);
};

let setSiteStyleOverrides = () => {
  if (document.getElementById("site-style-overrides")) return;

  const style = document.createElement("style");
  style.id = "site-style-overrides";
  style.textContent = siteStyleOverrides;
...
let cleanCvDateColumn = () => {
  ...
  document.querySelectorAll(".cv .date-column p.location").forEach((location) => {
    const text = location.textContent.replace(/\s+/g, " ").trim();
    location.textContent = "";

    const icon = document.createElement("i");
    icon.className = "fa-solid fa-map-pin iconlocation";
    icon.setAttribute("aria-hidden", "true");

    location.append(icon, document.createTextNode(text));
  });
};
...
let initTheme = () => {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (mode_toggle) {
      mode_toggle.closest(".toggle-container")?.remove();
    }
